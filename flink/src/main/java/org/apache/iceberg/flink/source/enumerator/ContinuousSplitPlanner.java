 ...
 */
class ContinuousSplitPlanner {

  private static final Logger LOG = LoggerFactory.getLogger(ContinuousSplitPlanner.class);
  ...
  public SplitPlanningResult planSplits(
    ...
    if (lastEnumeratedSnapshotId.isPresent()) {
      ...
      // first time
      final long startSnapshotId = getStartSnapshotId(table, contEnumConfig);
  ...
  @VisibleForTesting
  Optional<Long> findSnapshotIdByTimestamp(long timestamp, List<HistoryEntry> historyEntries) {
